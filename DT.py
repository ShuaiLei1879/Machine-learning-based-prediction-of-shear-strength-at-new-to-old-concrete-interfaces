...
def plot_learning_curves(model, X, y, kf, title='Learning Curves'):
    ...
    plt.legend(loc="best")
    plt.savefig('E:/Python_study/ProjectOutputs/DecisionTreeRegressor/learning_curves.png', dpi=400,
                bbox_inches='tight')
    plt.show()
...
def plot_loss_curves(model, X_train, y_train, X_test, y_test, step=1):
    ...
    plt.grid(True)
    plt.savefig('E:/Python_study/ProjectOutputs/DecisionTreeRegressor/Loss_Curves.png', dpi=400,
                bbox_inches='tight')
    plt.show()
...
grid_search = GridSearchCV(dt_regressor, param_grid, cv=kf, scoring='r2', n_jobs=-1)
...
train_results.to_csv("E:/Python_study/ProjectOutputs/DecisionTreeRegressor/train_predictions.csv", index=False)
...
test_results.to_csv("E:/Python_study/ProjectOutputs/DecisionTreeRegressor/test_predictions.csv", index=False)
...
plt.title('Actual vs Predicted with Decision Tree Regression')
plt.legend()
# 设置网格
plt.grid(True, which='both', axis='both', linestyle='--', linewidth=0.5, alpha=0.7)
# 设置坐标轴刻度指向内
plt.tick_params(axis='both', direction='in', length=6)
plt.savefig('E:/Python_study/ProjectOutputs/DecisionTreeRegressor/Actual vs Predicted with Decision Tree Regression.png', dpi=400,bbox_inches='tight')
plt.show()
...
plt.legend()
plt.savefig('E:/Python_study/ProjectOutputs/DecisionTreeRegressor/Residuals vs Predicted.png', dpi=400,bbox_inches='tight')
plt.show()
...
plt.savefig('E:/Python_study/ProjectOutputs/DecisionTreeRegressor/Q-Q Plot of Residuals.png', dpi=400,bbox_inches='tight')
plt.show()

# 在模型上绘制学习曲线
plot_learning_curves(best_dt, X_train, y_train, kf, title='Learning Curves for Decision Tree Regression')

# 调用函数绘制损失曲线
plot_loss_curves(best_dt, X_train, y_train, X_test, y_test, step=1)
