...
def plot_learning_curves(model, X, y, kf, title='Learning Curves'):
    ...
    plt.legend(loc="best")
    plt.savefig('E:/Python_study/ProjectOutputs/KNeighborsRegressor/learning_curves.png', dpi=400, bbox_inches='tight')
    plt.show()
...
def plot_loss_curves(model, X_train, y_train, X_test, y_test, step=1):
    ...
    plt.grid(True)
    plt.savefig('E:/Python_study/ProjectOutputs/KNeighborsRegressor/Loss_Curves.png', dpi=400, bbox_inches='tight')
    plt.show()
...
param_grid = {
    ...
    'weights': ['uniform', 'distance'],  # 加入weights参数
...
train_results.to_csv("E:/Python_study/ProjectOutputs/KNeighborsRegressor/train_predictions.csv", index=False)
...
test_results.to_csv("E:/Python_study/ProjectOutputs/KNeighborsRegressor/test_predictions.csv", index=False)
...
plt.title('Actual vs Predicted with KNeighborsRegressor')
plt.legend()
# 设置网格
plt.grid(True, which='both', axis='both', linestyle='--', linewidth=0.5, alpha=0.7)
# 设置坐标轴刻度指向内
plt.tick_params(axis='both', direction='in', length=6)
plt.savefig('E:/Python_study/ProjectOutputs/KNeighborsRegressor/Actual vs Predicted with KNeighborsRegressor.png', dpi=400, bbox_inches='tight')
plt.show()
...
plt.legend()
plt.savefig('E:/Python_study/ProjectOutputs/KNeighborsRegressor/Residuals vs Predicted.png', dpi=400, bbox_inches='tight')
plt.show()
...
plt.savefig('E:/Python_study/ProjectOutputs/KNeighborsRegressor/Q-Q Plot of Residuals.png', dpi=400, bbox_inches='tight')
plt.show()

# 在模型上绘制学习曲线
plot_learning_curves(best_knn, X_train, y_train, kf, title='Learning Curves for KNN Regression')

# 调用函数绘制损失曲线
plot_loss_curves(best_knn, X_train, y_train, X_test, y_test, step=1)

# # 使用SHAP进行模型解释
# explainer = shap.KernelExplainer(best_knn.predict, shap.sample(X_train, 200))
