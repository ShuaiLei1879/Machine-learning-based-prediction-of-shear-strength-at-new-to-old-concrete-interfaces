...
def plot_learning_curves(model, X, y, kf, title='Learning Curves'):
    ...
    # 保存并显示
    plt.savefig('E:/Python_study/ProjectOutputs/GradientBoostingRegressor/learning_curves.png',
                dpi=400, bbox_inches='tight')
    plt.show()

# 定义损失曲线（随 n_estimators 变化）的函数
def plot_loss_curves(model, X_train, y_train, X_test, y_test, max_estimators=2000, step=10):
    ...
    # 减小步长，绘制更多点，使曲线更精细
    for n_estimators in range(1, max_estimators + 1, step):
        # 设置迭代次数
    ...
    # 保存并显示
    plt.savefig('E:/Python_study/ProjectOutputs/GradientBoostingRegressor/Loss_Curves.png',
                dpi=400, bbox_inches='tight')
    plt.show()
...
# 尝试将特征列转换为数值型（非数值转为 NaN），最后一列默认为目标值
...
# 定义参数网格
param_grid = {
    'n_estimators': [100, 200, 400, 500, 600, 700, 800, 900, 1000],
    'learning_rate': [0.2, 0.01, 0.1, 0.001],
    'max_depth': [1, 3, 5, 7]
}
...
# 在训练集和测试集上进行预测
y_train_pred = best_gbrt.predict(X_train)
y_test_pred = best_gbrt.predict(X_test)

test_df = X_test_original.copy()  # 保留列名
test_df['True'] = y_test.values
test_df['Pred'] = y_test_pred
test_df.to_csv("E:/Python_study/ProjectOutputs/GradientBoostingRegressor/test_data_original_with_pred.csv", index=False)
...
train_results.to_csv("E:/Python_study/ProjectOutputs/GradientBoostingRegressor/train_predictions.csv", index=False)
...
test_results.to_csv("E:/Python_study/ProjectOutputs/GradientBoostingRegressor/test_predictions.csv", index=False)
joblib.dump(best_gbrt, 'E:/Python_study/ProjectOutputs/GradientBoostingRegressor/best_gbrt_model.joblib')
...
plt.title('Actual vs Predicted with GradientBoostingRegressor')
plt.legend()
# 设置网格
plt.grid(True, which='both', axis='both', linestyle='--', linewidth=0.5, alpha=0.7)
# 设置坐标轴刻度指向内
plt.tick_params(axis='both', direction='in', length=6)
plt.savefig('E:/Python_study/ProjectOutputs/GradientBoostingRegressor/Actual_vs_Predicted.png',
            dpi=400, bbox_inches='tight')
plt.show()
...
plt.legend()
plt.savefig('E:/Python_study/ProjectOutputs/GradientBoostingRegressor/Residuals_vs_Predicted.png',
            dpi=400, bbox_inches='tight')
plt.show()

# 绘制Q-Q图
plt.figure(figsize=(10, 6))
stats.probplot(np.concatenate([y_train - y_train_pred, y_test - y_test_pred]),
               dist="norm", plot=plt)
plt.title('Q-Q Plot of Residuals')
plt.savefig('E:/Python_study/ProjectOutputs/GradientBoostingRegressor/Q-Q_Plot_of_Residuals.png',
            dpi=400, bbox_inches='tight')
plt.show()

# 在模型上绘制学习曲线
plot_learning_curves(best_gbrt, X_train, y_train, kf, title='Learning Curves for GradientBoostingRegressor')

# 调用函数绘制损失曲线
plot_loss_curves(best_gbrt, X_train, y_train, X_test, y_test)
...
for param in tqdm(params_list, desc="Evaluating parameter combinations"):
    # 根据当前参数组合初始化一个新的 GradientBoostingRegressor
    model = GradientBoostingRegressor(random_state=10, **param)
    model.fit(X_train, y_train)
...
# 导出到 CSV 文件
output_path = 'E:/Python_study/ProjectOutputs/GradientBoostingRegressor/grid_search_test_scores.csv'
...
# ================== SHAP可解释性分析 ==================
# # 使用 TreeExplainer 进行模型解释
# explainer = shap.Explainer(best_gbrt, X_train)  # TreeExplainer 自动选择最佳解释器
